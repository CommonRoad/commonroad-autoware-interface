...
def add_ego_vehicle(
    ...
    ego_init_state = InitialState(time_step=cr_state_list[0].time_step,
    ...
    ego = DynamicObstacle(obstacle_id=42,
    ...
    start_time_step = min(state.time_step for state in cr_state_list if state.velocity > 0)
    ...
    for dyn_obs in scenario.dynamic_obstacles:
        if dyn_obs.initial_state.time_step < start_time_step:
            ...
            idx_first_state = next((i for i, state in enumerate(state_list) if state.time_step >= start_time_step), None)
